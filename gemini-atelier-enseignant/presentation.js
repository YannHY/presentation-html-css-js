(() => {
  ...
  const chapterMenuWrap = document.querySelector('.chapter-menu-wrap');
  ...
  chapters.forEach(chapter => {
    ...
    button.addEventListener('click', () => {
      goTo(slides.findIndex(slide => slide.dataset.chapter === chapter));
      closeChapterMenu();
      button.blur();
    });
  ...
  function closeChapterMenu() {
    ...
    chapterMenuToggle.setAttribute('aria-label', 'Afficher le sommaire des parties');
  }
  ...
  function pauseSlideMedia(exceptSlide = null) {
    slides.forEach(slide => {
      if (slide === exceptSlide) return;
      slide.querySelectorAll('audio, video').forEach(media => media.pause());
      slide.querySelectorAll('.lyria-music-toggle.is-playing').forEach(button => {
        button.classList.remove('is-playing');
        button.setAttribute('aria-pressed', 'false');
        button.setAttribute('aria-label', button.dataset.playLabel || 'Lire le morceau');
  ...
  function goTo(index, { updateHash = true, reveal = 'start' } = {}) {
    ...
    currentChapter.textContent = chapterLabels[slides[current].dataset.chapter];
    counter.textContent = `${String(current + 1).padStart(2, '0')} / ${String(slides.length).padStart(2, '0')}`;
    progress.style.width = `${((current + 1) / slides.length) * 100}%`;
    notesContent.textContent = slides[current].dataset.notes || '';
    pauseSlideMedia(slides[current]);
    updateBuildState();
    if (changed) { closeNotes(); closeShortcuts(); closeChapterMenu(); }
  ...
  chapterMenuToggle.addEventListener('click', () => {
    ...
    chapterMenuToggle.setAttribute('aria-expanded', String(open));
    chapterMenuToggle.setAttribute('aria-label', open ? 'Masquer le sommaire des parties' : 'Afficher le sommaire des parties');
  });
  notesToggle.addEventListener('click', () => {
    const open = notesPanel.classList.toggle('open');
    if (open) { closeShortcuts(); closeChapterMenu(); }
  ...
  shortcutsToggle.addEventListener('click', () => {
    ...
    if (open) { closeNotes(); closeChapterMenu(); }
  ...
  document.addEventListener('click', event => {
    if (!chapterMenuWrap.contains(event.target)) closeChapterMenu();
  });
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape') { closeNotes(); closeShortcuts(); closeChapterMenu(); return; }
  ...
  document.querySelectorAll('.lyria-music-card').forEach(card => {
    const button = card.querySelector('.lyria-music-toggle');
    const audio = card.querySelector('audio');
    const icon = button?.querySelector('i');
    if (!button || !audio || !icon) return;
    const setPlaying = playing => {
      button.classList.toggle('is-playing', playing);
      button.setAttribute('aria-pressed', String(playing));
      button.setAttribute('aria-label', playing ? (button.dataset.pauseLabel || 'Mettre en pause le morceau') : (button.dataset.playLabel || 'Lire le morceau'));
      icon.className = playing ? 'fa-solid fa-pause' : 'fa-solid fa-play';
    };
    button.addEventListener('click', async () => {
      pauseSlideMedia(card.closest('.slide'));
      if (audio.paused) {
        await audio.play();
        setPlaying(true);
      } else {
